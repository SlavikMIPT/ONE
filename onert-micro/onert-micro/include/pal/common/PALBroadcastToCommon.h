...
template <int N>
OMStatus BroadcastImpl(const NdArrayDesc<N> &input_desc, const uint8_t *input_data,
                       const NdArrayDesc<N> &output_desc, uint8_t *output_data, int indexes[N],
                       int dim, const int last_broadcasting_dim, const uint32_t type_size)
{
...
inline OMStatus BroadcastTo(const core::OMRuntimeShape &unextended_input_shape, const T *input_data,
                            const core::OMRuntimeShape &unextended_output_shape, T *output_data)
{
  ...
  {
    memcpy(reinterpret_cast<uint8_t *>(output_data), reinterpret_cast<const uint8_t *>(input_data),
           unextended_input_shape.flatSize() * sizeof(T));
    return Ok;
  }

  // Broadcasting using memcpy.
  int indexes[N] = {0};
  BroadcastImpl<N>(input_desc, reinterpret_cast<const uint8_t *>(input_data), output_desc,
                   reinterpret_cast<uint8_t *>(output_data), indexes, 0, last_broadcast_dim,
                   sizeof(T));
  return Ok;
}
} // namespace pal
